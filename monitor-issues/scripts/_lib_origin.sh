# Shared origin-resolution helpers for monitor-issues's scripts.
#
# This file is meant to be SOURCED, not executed directly — it defines
# globals/functions used by monitor_issues.sh.
#
# Scope: this sourcing is local to the monitor-issues skill only. Other
# skills (new-issue, auto-new-issue, auto-fix-issue, auto-fix-all,
# auto-monitor-pr, auto-monitor-issue-pr, etc.) intentionally keep their
# own independent copies of similar helpers so each skill folder remains
# self-contained and portable on its own — that cross-skill boundary is
