 ...
 * each field's shell original exactly: `git.safe_branch` and
 * `plan-issues.*` from local `.claude/state/arcanum-config.json`
 * (mirroring `arcanum/_lib/safe_branch.sh`'s `safe_branch_get`), and
...
class RepoConfig {
   ...
   * different file and a namespaced key from `getSafeBranch`/
   * `getPlanIssuesRetryConfig` above, and (matching the shell script's
   * documented behavior) with no legacy `.claude/configuration/auto-fix-all.json`
