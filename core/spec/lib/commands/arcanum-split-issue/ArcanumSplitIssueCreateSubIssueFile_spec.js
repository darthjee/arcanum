  ...
  describe('#run', () => {
    ...
    describe('sub-issue counting', () => {
      it('starts a fresh id at count 01', async () => {
        ...
        const written = await readFile(path.join(fixture.repoPath, ISSUES_DIR, `${ISSUE_ID}_01_first_sub_issue.md`), 'utf8');
      ...
      it('picks up an out-of-band file created outside the tool', async () => {
        ...
        const result = await instance.run(ISSUE_ID, 'Fourth Sub Issue', fixture.seedFilePath);
      ...
      it('never reuses a count freed by a deleted file (gap-tolerant)', async () => {
        ...
        const result = await instance.run(ISSUE_ID, 'Fifth Sub Issue', fixture.seedFilePath);
      ...
      it('skips filenames whose count segment is not numeric', async () => {
        ...
        const result = await instance.run(ISSUE_ID, 'Next Sub Issue', fixture.seedFilePath);
    ...
    describe('success output contract', () => {
      it('writes "# <title>\\n\\n<body>" and returns FILE=<path>\\n', async () => {
        ...
        const written = await readFile(path.join(fixture.repoPath, expectedFile), 'utf8');
      ...
      it('copies the body file bytes verbatim, without trailing-newline normalization', async () => {
        ...
        const written = await readFile(path.join(fixture.repoPath, ISSUES_DIR, `${ISSUE_ID}_01_my_title.md`), 'utf8');
