import { seedEngineMode } from '../utils/engineMode.js';
 ...
 * building a fresh git fixture repo, seeding `engine.mode`, letting the
 * caller prepare whatever per-case fixture state a given subcommand
 * needs, running the shim, and handing the result to the caller's own
 * assertion callback for that mode.
 ...
 *   - per-case fixture setup, run after `seedEngineMode`. Returns the
 *   arguments to append after `shimScript` when invoking `runCommand`,
 *   an optional environment to run it with, and an optional `cleanup`
 *   (e.g. a `fakeGh.cleanup()`) run alongside the fixture repo's own
 *   teardown.
