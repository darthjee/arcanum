...
describe('issue-state parity (shell vs. native) — append-json', () => {
  ...
  describe('append-json on a field that does not exist yet', () => {
    it('produces byte-identical resulting state-file content, stdout, and exit code', async () => {
      const { shell, native } = await runBoth(['append-json', '42', 'tags', '"a"'], shellRepo, nativeRepo);
  ...
  describe('append-json on a field that is already an array', () => {
    it('produces byte-identical resulting state-file content, stdout, and exit code', async () => {
      await runBoth(['set-json', '42', 'tags', '["a","b"]'], shellRepo, nativeRepo);

      const { shell, native } = await runBoth(['append-json', '42', 'tags', '"c"'], shellRepo, nativeRepo);
