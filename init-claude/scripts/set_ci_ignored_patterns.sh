...
#
# Requires at least one pattern. Writes the full given list (replacing
# any existing value) as a JSON array under
# .claude/configuration/arcanum-repo-config.json's "auto-fix-all"
# namespace key, "ignored_check_patterns" field — via
...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <pattern-1> [<pattern-2> ...]" >&2
  exit 1
fi
