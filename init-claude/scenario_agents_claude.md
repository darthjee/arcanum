# Scenario: AGENTS.md and CLAUDE.md exist, copilot-instructions.md absent

`AGENTS.md` and `CLAUDE.md` exist but `.github/copilot-instructions.md` does not. The content of both will be consolidated into `AGENTS.md`, then `CLAUDE.md` will point to it and `copilot-instructions.md` will be created.

## Step 1 — Announce the intent

Tell the user:

```
Found AGENTS.md and CLAUDE.md. Their contents will be consolidated into AGENTS.md.
The following changes will be made:
- AGENTS.md                          (updated with consolidated content)
- CLAUDE.md                          (replaced, pointing to AGENTS.md)
- .github/copilot-instructions.md    (created, pointing to AGENTS.md)
```

## Step 2 — Ask for confirmation

Ask:

```
Shall I proceed?
```

Wait for the user's response.

- If the user confirms: proceed.
- If the user declines: stop.

## Step 3 — Read both files

Read the full content of:
- `AGENTS.md`
- `CLAUDE.md`

## Step 4 — Consolidate into AGENTS.md

Merge both contents into a single coherent document. Follow these rules:
- Remove duplicate information, keeping only one copy
- Preserve all unique information from both files
- Use the structure and headings that best represent the combined content
- Do not add a preamble explaining the merge — just produce clean, unified instructions

Overwrite `AGENTS.md` with the consolidated content.

## Step 5 — Replace CLAUDE.md

Overwrite `CLAUDE.md` with:

```markdown
See [AGENTS.md](AGENTS.md) for project instructions.
```

## Step 6 — Create .github/copilot-instructions.md

Ensure the `.github/` directory exists, then create `.github/copilot-instructions.md` with:

```markdown
See [AGENTS.md](../AGENTS.md) for project instructions.
```

## Step 7 — Confirm completion

Tell the user:

```
Done! Migration complete:
- AGENTS.md — consolidated content from AGENTS.md and CLAUDE.md
- CLAUDE.md — now points to AGENTS.md
- .github/copilot-instructions.md — created, pointing to AGENTS.md

Edit AGENTS.md to keep your project instructions up to date — both Claude and Copilot will read from it.
```
