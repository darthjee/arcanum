#!/usr/bin/env bash
# Migration 003 (next): seed
# .claude/configuration/arcanum-repo-config.json's "git"."email" key —
# the repo-wide fallback for the per-agent "Co-Authored-By" email
# pattern used by commit_change.sh/commit_issue.sh/commit_plan.sh (see
# arcanum/_lib/agent_email.sh), for repos that want every contributor
# to share the same pattern rather than configuring it locally per
# clone (see migration 002 in this same folder). See 003.md for a
...
cmd_run() {
  ...
  if ! ( exec 3< /dev/tty ) 2>/dev/null; then
    ...
    # nothing to write.
    [[ -n "$guess" ]] && repo_config_write ".claude/configuration/arcanum-repo-config.json" "" "git" "email" "\"${guess}\""
    return 0
  fi
  ...
  if [[ -n "$guess" ]]; then
    ...
    case "$choice" in
      [Yy]*)
        repo_config_write ".claude/configuration/arcanum-repo-config.json" "" "git" "email" "\"${guess}\""
        ;;
      [Tt]*)
        printf 'Email pattern to use (e.g. you+{agent}@example.com): '
        local typed
        read -r typed < /dev/tty
        repo_config_write ".claude/configuration/arcanum-repo-config.json" "" "git" "email" "\"${typed}\""
        ;;
      *)
