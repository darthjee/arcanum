...
# folders are pending, sorted ascending, under the dual-pointer scheme
# (committed version + local-only version — see
...
#
# A version folder is pending if either axis still has unsatisfied
# work in it: it has at least one "repo"-scoped manifest entry and the
...
# strictly less than the folder. This mirrors the single-loop,
# dual-gating design: a folder can be revisited by one clone for its
# local-scoped entries alone, long after its repo-scoped entries were
# already satisfied (and shipped) via another clone's commit.
#
...
# _pending_versions <committed_version> <local_version>
#   Prints, one per line, every version folder name under
#   ${MIGRATIONS_SCRIPT_DIR}/repos/ whose manifest has at least one
#   "repo"-scoped entry beyond <committed_version> or at least one
#   "local"-scoped entry beyond <local_version>, sorted ascending.
...
#   migrations.json.
_pending_versions() {
  local committed="$1" local_version="$2"
