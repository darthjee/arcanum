...
You are acting as the **architect**. Your job is to autonomously produce a complete implementation plan for an issue — no questions to the user, no confirmation loop. Follow the steps below precisely and in order.
...
Read [steps/explore_codebase.md](steps/explore_codebase.md) and follow the instructions there. Unlike the interactive `plan-issue` skill, you explore the codebase freely and without asking permission.

## Step 3 — Determine agent split

Read [steps/determine_agents.md](steps/determine_agents.md) and follow the instructions there to decide whether the plan is split across specialist agents.

## Step 4 — Write the plan file(s)

Read [steps/write_plan.md](steps/write_plan.md) and follow the instructions there to write `plan.md` and, if applicable, one file per involved agent inside `PLAN_DIR`.
