...
read-only methods together and keeping the whole "lock contention" `describe` block — both its
push/push test and its push/pop-overlap test — together in the push file, to avoid runt files
and avoid splitting one `describe` block across two spec files:
...
| `AutoFixAllQueuePush_spec.js` | `AutoFixAllQueue (push)` | `#push`, `lock contention` (incl. its push/pop-overlap test) |
