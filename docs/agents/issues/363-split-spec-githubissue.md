...
|---|---|---|
| `GithubIssueFetch_spec.js` | `GithubIssue#fetch` | the current `#fetch` block (7 `it`s: happy path, REST URL/auth header, label→tag mapping, title slugging, fetch-failure, network-error, auth-failure, timeout) |
