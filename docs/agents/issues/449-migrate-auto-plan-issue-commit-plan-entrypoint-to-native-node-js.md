# Issue: Migrate auto-plan-issue-commit-plan entrypoint to native Node.js
...
## Benefits
Brings `commit_plan.sh` in line with the rest of the `auto-plan-issue`/`auto-fix-issue` family already migrated to native Node.js, adds unit and parity test coverage matching the established pattern, and keeps `engine_dispatch.sh` routing consistent between `engine.mode=native` and `engine.mode=shell`.
