...
- **`core/spec/support/utils/runCommand.js`** gains:
  ...
  - `expectParity` relocated here from `githubParitySetup.js` (where it's currently defined and exported) — it's fully generic (just compares stdout/code), and the queue spec currently repeats the same two `expect()` lines inline 6+ times instead of using it. Of the 8 existing `core/spec/bin/autoFixAllGithubParity/*.js` files, the 6 that import `expectParity` get that import repointed to pull from `runCommand.js` directly (the other 2, `cleanup_branch_spec.js` and `engine_dispatch_spec.js`, don't use it); `githubParitySetup.js` no longer defines or re-exports it.
