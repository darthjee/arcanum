...
Split along the same scenario groupings the sibling shell-parity suite
(`core/spec/bin/autoFixAllWaitCiParity_spec.js`, itself split under issue #350) uses, as flat
sibling files in `core/spec/lib/commands/auto-fix-all/`:
...
- The `bin/autoFixAllWaitCiParity_spec.js` split (issue #350) — separate, already covered.
