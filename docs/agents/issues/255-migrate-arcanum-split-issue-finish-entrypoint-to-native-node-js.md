...
`arcanum-split-issue/scripts/finish.sh`

Finishes up after all sub-issues have been pushed: relabels the parent issue (Planning -> Split), deletes local working files (`docs/agents/issues/<id>-*` and `<id>_*`), and releases the working tree to the configured safe branch.
...
- Calls `arcanum-split-issue/scripts/github.sh mark-split` — this is the **out-of-batch** thin wrapper delegating to `arcanum/_lib/github_issue.sh` (tracked separately as `github-issue`, only partially migrated so far via `github-issue-create`/`github-issue-info`). `mark-split` itself is not yet natively migrated, so this can shell out to it as-is for now.
