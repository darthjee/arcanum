...
`createGitFixture` verbatim into a shared support module (e.g.
`core/spec/support/factories/arcanumUpdateRunUpdateParity.js`), imported by both new files —
this is the piece that actually avoids the ~160-line duplication a naive per-describe split
would cause.
