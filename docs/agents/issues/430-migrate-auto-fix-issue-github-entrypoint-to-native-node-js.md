...
8. Add four separate keys to `arcanum/_lib/migration-status.json` — `"auto-fix-issue-github-info"`, `"auto-fix-issue-github-pr-create"`, `"auto-fix-issue-github-pr-view"`, `"auto-fix-issue-github-pr-ready"` — each set to `true`. `engine_dispatch`'s native-availability check is keyed on the exact `<command>` string passed to it per call, so a single shared `auto-fix-issue-github` key (unlike this issue's earlier draft) would not actually gate per-subcommand routing correctly once `github.sh` is rewritten into real per-subcommand shims (step 2).
