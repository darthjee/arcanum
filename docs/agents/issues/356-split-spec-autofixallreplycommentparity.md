...
Move `runCommand`, `git`, and `seedGithubLikeRepo` into a shared support module at
`core/spec/support/factories/autoFixAllReplyCommentParitySetup.js` (following the existing
`autoFixAllConfigParitySetup.js` / `autoFixAllCheckoutFromMainParitySetup.js` naming
convention), imported by all three new files. Behavior copied verbatim, including the
existing imports from `../support/utils/` (`fakeGhBin`, `gitFixtureRepo`, `seedOriginUrl`,
`tempDir`) and the module-level constants (`SHELL_SCRIPT`, `NATIVE_BIN`,
`FAKE_FETCH_PRELOAD`, `ID`, `ARGS_TAIL`) those helpers depend on.
