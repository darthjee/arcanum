...
support module, following the established `<Name>ParitySetup.js` naming convention used by
the prior splits (e.g. `arcanumSplitIssueCreateSubIssueParitySetup.js`):
`core/spec/support/factories/arcanumSplitIssuePushSubIssuesParitySetup.js`. Imported by both
new files (`argument_validation_spec.js` only needs `runCommand`/`runBoth`;
`push_behavior_spec.js` needs all four). Generic utilities (`createGitFixtureRepo`,
`createTempDir`/`removeTempDir`) keep being imported directly from `support/utils/`, not
re-exported from the setup module. Behavior copied verbatim.
