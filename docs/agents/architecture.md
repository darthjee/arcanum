...
1. **Bootstrap** (`arcanum/update/bootstrap.sh`) — pode ser invocado tanto via `curl | bash` (sempre busca a lógica mais nova) quanto diretamente a partir de uma instalação já existente (`bash <install-dir>/arcanum/update/bootstrap.sh`). Diferente do `install/bootstrap.sh`, não tem uma versão padrão embutida: quando `ARCANUM_VERSION` não é definido, consulta `GET https://api.github.com/repos/${ARCANUM_REPO}/releases/latest` (parseado com `grep`/`sed`, sem `jq`, para manter essa etapa livre de dependências) para resolver a última release publicada. Também resolve `TARGET` e o `REPO` padrão antes de baixar qualquer coisa: se `${BASH_SOURCE[0]}` aponta para um arquivo real já dentro de uma instalação (dois diretórios acima existe `arcanum.json`), usa esse diretório como `TARGET` e lê `repo` de `arcanum.json` como default de `ARCANUM_REPO` (uma fork instalada continua atualizando a partir da própria fork, a menos que `ARCANUM_REPO` seja definido explicitamente); senão, cai para `ARCANUM_TARGET` se definido, ou deixa `TARGET` vazio para o `updater.sh` perguntar interativamente. Exporta `REPO`/`VERSION`/`TARGET` antes do `exec` para `updater.sh`.
2. **Updater** (`arcanum/update/updater.sh`) — vem dentro do zip de release. Lê `arcanum.json` existente no alvo (versão/repo/manifest antigos); se a versão já bate com a nova, encerra com uma mensagem "already on X.Y.Z" sem tocar em nada. Caso contrário: copia a árvore nova por cima do alvo (`cp -R`, igual ao instalador), calcula o conjunto de remoção como a diferença entre o `manifest` antigo (de `arcanum.json`) e o `MANIFEST` novo (embutido no zip) — cada caminho candidato à remoção é validado antes de qualquer `rm` (rejeita caminho absoluto ou com segmento `..`; resolve o caminho físico via `cd`/`pwd` do diretório contendo o arquivo e confirma que o resultado continua estritamente dentro do alvo) — e só então grava o novo `arcanum.json`, por último, o que torna uma falha no meio do processo seguramente reexecutável. Limpa seu próprio diretório de trabalho ao final via `trap ... EXIT` (diferente do `install/bootstrap.sh`, que nunca limpa o seu, aceitável para uma operação de execução única; `update` tende a rodar repetidamente).
...
| `.claude/configuration/auto-fix-all.json` | Committed, shared configuration for the `auto-fix-all` skill. Controls which CI check names are ignored when deciding pass/fail. No longer holds `clear_context` — that personal, frequently-toggled run-time preference lives in `.claude/state/auto-fix-all-config.json` instead. | `{"ignored_check_patterns": ["<substring>", ...]}` |
| `.claude/state/auto-fix-all-config.json` | Per-checkout, gitignored run-time state for `auto-fix-all`. Holds the `clear_context` toggle flipped by `/toggle-clear-context`, and the `finish_on_empty_queue` toggle (opts a run into stopping cleanly once its queue drains instead of blocking forever on `queue.sh wait-next`) — both writable via `init-claude`'s `setup_auto_fix_all_config.md` step, `finish_on_empty_queue` has no standalone toggle skill. Both are kept out of the committed `.claude/configuration/auto-fix-all.json` so they can't be accidentally swept into unrelated commits. `auto-fix-all/scripts/config.sh` routes both keys' reads/writes here; every other key still goes to the committed configuration file. | `{"clear_context": true\|false, "finish_on_empty_queue": true\|false}` |
