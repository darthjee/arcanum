...
`id` is zero-padded, matching the paired `<id>.sh`; `type` is only ever `"script"` for now; `file` is the script's filename within the same version folder; `skippable` moved into the manifest (manifest-driven entries no longer shell out to `<file> config` to learn it — that subcommand is only still called for legacy, glob-discovered entries); `applies_to` is `"repo"` or `"local"` — no combined `"both"` value, a change needing both kinds of effects is authored as two separate entries (see docs/guides/arcanum-repo-version.md for why). The full contract is documented as a header comment in `arcanum/migrations/_manifest.sh`.

**Manifest vs. legacy discovery** — `arcanum/migrations/_manifest.sh`'s `_manifest_entries <version_dir>` is the single place this decision is made: it reads `migrations.json` when present, or falls back to globbing `NNN.sh` (sorted by filename, implicit `applies_to: "repo"`, `skippable` sourced live from `<file> config`) when it's absent. Every other script in the chain below goes through this helper rather than re-implementing either path itself.
...
**The runner chain** — `arcanum/migrations/run.sh` (top-level entry point) → `select_version.sh` (when the user picks specific versions) → `update_per_version.sh` (one version folder) → `update_per_file.sh` (one script) — mirrors the confirmation-gate pattern used elsewhere in arcanum (explicit `[A]ll`/`[N]one`/`[S]elect`, never a silent unattended sweep by default). Every level also offers `[C]hat`: choosing it hands control back to the calling skill via a dedicated exit code `3` (propagated immediately up the chain the same way the existing halt code `2` already is), printing `CHAT_CONTEXT=<version>[/<file>]` identifying what the user asked about. `run.sh` (and every script below it) takes an optional, explicit `--repo <path>` argument — defaulting to cwd when omitted, for backward-compatible direct terminal use — so target-repo-relative paths (`.claude/configuration/arcanum-repo-config.json`, `.claude/state/arcanum-config.json`, `.claude/state/arcanum-errors.json`) never depend on ambient cwd being trustworthy; the install location itself stays self-derived via `BASH_SOURCE`, never made explicit. `/arcanum-migrate` drives `run.sh`'s bare/interactive form directly, in a single call, letting bash own the whole `/dev/tty` prompt chain — TTY-through-a-tool-call is confirmed to work reliably — and only re-enters chat via the `[C]hat`/exit-`3` escape hatch, resuming afterward through the still-present non-interactive `check`/`apply --all|--none|--select <version>` form. `/arcanum-update` is unchanged by this and still uses its own check/chat-confirm/apply split (a separate, still-valid choice for that skill, not part of this pattern). Errors are collected in `.claude/state/arcanum-errors.json` (overwritten fresh per `run.sh` invocation, not appended) and printed at the end of the run regardless of whether any were skippable.

**Dual-pointer version tracking, single loop:** the repo's current migration progress is tracked by *two* independent pointers — the committed one, `.version` in `.claude/configuration/arcanum-repo-config.json` (see the table above), and a local-only one, `.migrations.version` in `.claude/state/arcanum-config.json` (gitignored, per clone/machine). A `"repo"`-scoped manifest entry is satisfied once the committed pointer reaches its version folder (one clone's commit satisfies every clone that pulls it); a `"local"`-scoped entry is satisfied only once *that clone's own* local pointer does, since `.claude/state/` never travels through git — this is what fixes the false "fully caught up" signal a second clone used to get when a migration's only effects lived in local state. `arcanum/migrations/_pending_versions.sh` computes the pending-versions list as the union of both axes (a version is pending if it has a `repo` entry beyond the committed pointer, or a `local` entry beyond the local pointer) and `update_per_version.sh` walks each pending version's manifest as one single loop, in `id` order — not two separate per-scope passes — so entries stay in their authored relative order even when a `local` entry's effects depend on an earlier `repo` entry's in the same version, or vice versa. Already-satisfied entries (per whichever pointer their scope answers to) are silently skipped, not even shown. Both pointers are stamped by `init-claude` (`init-claude/scripts/stamp_arcanum_version.sh`) and advanced by `update_per_version.sh` **once per fully-processed manifest** (after its whole entry loop completes without halting) — never per-file, fixing a pre-existing bug where the committed pointer used to advance on every individual successful migration file, even when a later file in the same version subsequently halted. The committed pointer advances only if the manifest has at least one `repo`-scoped entry; the local pointer only if it has at least one `local`-scoped entry — a version whose manifest is entirely one scope never touches the other pointer. See `docs/guides/arcanum-repo-version.md` for the missing/invalid-semver fallback rules and the full two-pointers rationale, and `docs/guides/arcanum-repo-config.md` for the config-file-pair story these two pointers piggyback on.
