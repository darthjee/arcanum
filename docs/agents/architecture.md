...
| `.claude/state/auto-fix-all-config.json` | Per-checkout, gitignored run-time state for `auto-fix-all`. Holds the `clear_context` toggle flipped by `/toggle-clear-context`, kept out of the committed `.claude/configuration/auto-fix-all.json` so it can't be accidentally swept into unrelated commits. `auto-fix-all/scripts/config.sh` routes `clear_context` reads/writes here; every other key still goes to the committed configuration file. | `{"clear_context": true\|false}` |
