...
# the <shell_script> argument to engine_dispatch for the "info"
...
#
# Usage: github_shell_info.sh <repo_path>
set -euo pipefail

exec bash "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/github_shell.sh" info "$@"
