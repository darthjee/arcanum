...
# the <shell_script> argument to engine_dispatch for the "pr-ready"
...
#
# Usage: github_shell_pr_ready.sh <repo_path>
set -euo pipefail

exec bash "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/github_shell.sh" pr-ready "$@"
